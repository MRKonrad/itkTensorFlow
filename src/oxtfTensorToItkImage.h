...
namespace oxtf {
    ...
    public:
        ...
        TensorToImage() = default;
        virtual ~TensorToImage() = default;


        template<typename TensorPixelType, typename ImagePixelType>
        static
        int
        convertTensorBufferToImageBuffer(std::vector<std::int64_t> dims, TensorPixelType *tensorBuffer,
                                         ImagePixelType *pixelBuffer) {
...
//                            static_cast<TensorPixelType>(tensorBuffer[ x * dims[2] * dims[3] + y * dims[3] + z ]);
         ...
         */
        static
        int convert(const TF_Tensor *inputTensor, ImageType *outputImage) {
